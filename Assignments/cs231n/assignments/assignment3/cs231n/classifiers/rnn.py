...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
          hT, cache_rnn = rnn_forward(x_embed, h0, Wx, Wh, b)
        elif self.cell_type == 'lstm':
          hT, cache_lstm = lstm_forward(x_embed, h0, Wx, Wh, b)
        else:
          assert False, "Cell type is wrong!"
        scores, cache_temp = temporal_affine_forward(hT, W_vocab, b_vocab)
    ...
    def sample(self, features, max_length=30):
        ...
        ht = features @ W_proj + b_proj
        ct = np.zeros_like(ht)
        captions[:, 0] = self._start * np.ones(N, dtype=np.int32)
        for t in range(1, max_length):
          x_embed, _ = word_embedding_forward(captions[:, t-1], W_embed)
          if self.cell_type == 'rnn':
          ...
          captions[:, t] = scores.argmax(axis=-1).reshape((N,))
