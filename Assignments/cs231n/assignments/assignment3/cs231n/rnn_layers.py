...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for t in range(T):
        h[:, t, :], cache_t = rnn_step_forward(x[:, t, :], ht, Wx, Wh, b)
        ht = h[:, t, :]
...
def rnn_backward(dh, cache):
    ...
    for t in range(T-1, -1, -1):
        dx[:, t, :], dh0, dWx_t, dWh_t, db_t = rnn_step_backward(
            dh[:, t, :]+dh0, cache[t])
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    s = sigmoid(a[:, 0:3*H])
    g = np.tanh(a[:, 3*H:])
    i, f, o = s[:, 0:H], s[:, H:2*H], s[:, 2*H:3*H]
    next_c = f * prev_c + i * g
    next_h = o * np.tanh(next_c)

    cache = (x, prev_h, prev_c, next_c, a, Wx, Wh)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    x, prev_h, prev_c, next_c, a,  Wx, Wh = cache
    ...
    da[:, 0:3*H] *= sigmoid(a[:, 0:3*H]) * (1 - sigmoid(a[:, 0:3*H]))
    da[:, 3*H:] *= 1 - np.tanh(a[:, 3*H:])**2
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    cache = []
    for t in range(T):
        h[:, t, :], ct, cache_t = lstm_step_forward(
            x[:, t, :], ht, ct, Wx, Wh, b)
        ht = h[:, t, :]
        cache.append(cache_t)
...
def lstm_backward(dh, cache):
    ...
    db = np.zeros(4 * H)
    for t in range(T-1, -1, -1):
        dx[:, t, :], dh0, dct, dWx_t, dWh_t, db_t = lstm_step_backward(
            dh[:, t, :]+dh0, dct, cache[t])
        dWx += dWx_t
        dWh += dWh_t
        db += db_t
