...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        total_loss += - torch.log(numerator/denominator1) - \
            torch.log(numerator/denominator2)
...
def sim_positive_pairs(out_left, out_right):
    ...
    pos_pairs = (out_left[:, None, :] @ out_right[:, :, None]).squeeze(1)
    pos_pairs /= (torch.linalg.norm(out_left, dim=1) * torch.linalg.norm(out_right, dim=1)).unsqueeze(1)
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_pair = sim_positive_pairs(out_left, out_right)
    ...
    numerator = torch.exp(sim_pair / tau).repeat((2, 1))
